...
def clear() -> None:
    ...
    if STATE_PATH.exists():
        STATE_PATH.unlink()
    indexed_files = STATE_PATH.parent / "indexed_files.json"
    if indexed_files.exists():
        indexed_files.unlink()
    VectorIndex(STATE_PATH.parent).clear()

    # Clear created files under the sandbox (leaving subdirectories like corpus/ and papers/ intact)
    sandbox = STATE_PATH.parent / "sandbox"
    if sandbox.exists() and sandbox.is_dir():
        for item in sandbox.iterdir():
            if item.is_file() and item.name not in (".gitignore", ".DS_Store"):
                try:
                    item.unlink()
                except Exception:
                    pass

    # Clear all artifact store files under state/artifacts
