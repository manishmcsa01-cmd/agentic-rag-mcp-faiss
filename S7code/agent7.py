...
async def run(query: str) -> str:
    ...
    # Pass UTF-8 encoding env vars to the MCP subprocess so that Windows
    # doesn't default to cp1252 on the stdio pipes (causes charmap errors
    # when LLM responses contain em-dashes, curly quotes, or other Unicode).
    mcp_env = {
        **os.environ,
    ...
    try:
        ...
        # anyio raises an ExceptionGroup during MCP stdio_client teardown
        # even when the agent loop already produced a valid answer. If we
        # have a final_answer, treat the teardown noise as non-fatal.
        if final_answer:
            print(f"[agent7] MCP teardown raised ExceptionGroup (suppressed — answer already captured): {eg.exceptions[0]}")
        else:
            # No answer yet — surface the first real sub-exception.
